...
/*static*/ bool fNot16bit=false;

ImageMap Image_Map;											//DAW 27Oct98

UByte *ReadLong(UByte *c, SLong &number)
{
...
UByte *ReadWord(UByte *c, SWord &number)
{
	number=(c[0]<<8)+c[1];
	c+=2;
	return(c);
}
SLong SearchIFFHunks(SLong count,IFFHunkSearch* pSearch,UByte* ptr)
{
...
//#define	_IMAP_MEM_DEBUG_


#ifdef	_IMAP_MEM_DEBUG_

static	bool	memtable[512][512];
...
ImageMap::ImageMap()
{
	...
	total_piloted_ac_mem = 0;									//RJS 24Jun98
	total_cockpit_mem = 0;										//RJS 24Jun98
	DoingAC = FALSE;
	DoingCock = FALSE;
	DoingPilotedAC = false;
...
ImageMap::~ImageMap()
{
	...
	if (dfp)
	{
		...
		fprintf(dfp,"Piloted:\n");
		fprintf(dfp,"\tTotal a/c:\t%d\n",total_piloted_ac_mem);
		fprintf(dfp,"\tTotal cockpit:\t%d\n",total_cockpit_mem);
		fclose(dfp);
	}
#endif
...
ImageMapDescPtr ImageMap::GetImageMapPtr(ImageMapNumber imagemap_number,Bool	skipmap)
{
	...
	if(superimagemaptable[dir_no].imagemaptable[file_no]==NULL)	//CSB 10/11/98	FAILS HERE !!!!
	{
		...
		if ((real_imagemap_number != imagemap_number) && !fNot16bit) //PD 03/12/99
		{
			is16 = TRUE;
...
//DeadCode MS 26Sep00 	if (file_no == mapoff)
//DeadCode MS 26Sep00 		INT3;

	if ((realmapno != imageno) && !fNot16bit) //PD 03/12/99
...
//dead	File_Man.DiscardDirList(DIR_COCKPITS);
//dead	File_Man.DiscardDirList(DIR_COCKPT16);
...
Bool ImageMap::LoadImageMap(SuperMap*	supermap, UWord file_no,Bool is16bit, Bool	skipmap)
{
	...
	assert (ULong(supermap->imagemaptable[file_no])!=0xfdfdfdfd);
...
//DeadCode RJS 27Oct98 			UWord		fnum = DIR_IMAGEMAP + file_no;			//RJS 19Oct98
			...
			SLong	imgwidth = imgp->w;
			SLong	imgheight = imgp->h;

			dbg_total_mem = total_image_mem;

 			if (DoingAC)
 			{
 				total_ac_mem += imgwidth*imgheight;
// 				fprintf(dfp,"AC:   %s ..... %d\n",ImapNames[imagemap_number],imgwidth*imgheight);
 			...
 			else
 			{
 				if (DoingCock)
 				{
 					total_cockpit_mem += imgwidth*imgheight;
// 					fprintf(dfp,"Cock: %s ..... %d\n",ImapNames[imagemap_number],imgwidth*imgheight);
 				}
				else
				{
					if (DoingPilotedAC)
						total_piloted_ac_mem += imgwidth*imgheight;
//					else
//						fprintf(dfp,"Gen:  %s ..... %d\n",ImapNames[imagemap_number],imgwidth*imgheight);
				}
 			}
...
//DeadCode RJS 20Oct00 	UByte* start=buffer;
	...
	if (tmp)
	{
		tmp = ReadWord(tmp,picwidth);
		tmp = ReadWord(tmp,picheight);
		tmp += 5;												//RJS 26Mar98
		if (*tmp)												//RJS 17May00
			isMasked = 1;										//RJS 17May00
	}
	...
	if (tmp)
	{
		...
		tmp = ReadLong((UByteP)(tmp-4),blocksize);
	...
	if (tmp)
	{
		...
		if (alpha)
		{
//DeadCode RJS 17May00 			blendType = BLEND_AVERAGEALPHA;						//RJS 16May00
...
#ifdef	_IMAP_MEM_DEBUG_
	if (superimagemaptable[i].imagemaptable[file_no])
	{
		if (memtable[i][file_no])
			INT3;

		memtable[i][file_no] = true;
	}
#endif
