...
//Evaluate functionality:
	...
	int&		UsesGlobRef();
	Expr*		UsesGlobRef(int	globrefind);
	bool		DeRefGlobRefs();
	EventVal*	UsesUID(int	uniqueid,bool immediateisuid=false);
//Standard structure making:
	...
	void	MakeIcptGRExpr(SavedGlobrefs a,SavedGlobrefs b,bool ispercent=true);
...
virtual	int&		UsesGlobRef()	{return EventVal::minus1;}
virtual	Expr*		UsesGlobRef(int	globrefind)	{return NULL;}
virtual	EventVal*	UsesUID(int	uniqueid)	{return NULL;}
virtual	int			DeRefGlobRefs()=0;
...
inline	Expr*		EventVal::UsesGlobRef(int GRI)
{
	if (complex)
		return complex->UsesGlobRef(GRI);
	else
		return NULL;
}
inline	EventVal*		EventVal::UsesUID(int UID,bool immediateisUID)
{
	if (complex)
		return complex->UsesUID(UID);
	else
		if (immediateisUID && value==UID)
			return this;
		else
			return NULL;
...
inline	bool	EventVal::DeRefGlobRefs()
{
	if (complex)
	{
		int grresult=complex->DeRefGlobRefs();
		if (grresult==GOT_UNDEFINED_VAL)
			return false;
		else
		{
			delete complex;
...
struct	GlobRefExpr:Expr
{
	...
	Expr*		UsesGlobRef(int GRI){if (GRI==globrefind) return this; else return NULL;}
	int			DeRefGlobRefs()		{return Evaluate(0);}
//	EventVal*	UsesUID(int UID)
	int			Evaluate(int);
...
struct	UIDExpr:Expr
{
	...
	UniqueID	EvalW(Coords3D& c);
	UniqueID	EvalA(HP& c);
	int&		UsesGlobRef()		{return val.UsesGlobRef();}
	Expr*	UsesGlobRef(int GRI){return val.UsesGlobRef(GRI);}
	int			DeRefGlobRefs()		{val.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
	EventVal*	UsesUID(int UID)	{return val.UsesUID(UID,true);}
	void	Save(FILE* f,char* fname,int tabs,EnumListNames e);
	void	Save(BOStream& f,char* fname,int tabs,EnumListNames e);
	Expr*		Clone()
	{
...
struct	RndExpr:Expr
{
	...
	char*		FieldName(int l)	{	switch (l)
										...
										case 1:	return "step size";
										case 2:	return "rnd(steps)";
										}
										return "";
									}
	...
	void	Save(FILE* f,char* fname,int tabs,EnumListNames e);
	void	Save(BOStream& f,char* fname,int tabs,EnumListNames e);
	Expr*	UsesGlobRef(int GRI);
	int			DeRefGlobRefs()		{base.DeRefGlobRefs();stepsize.DeRefGlobRefs();maxsteps.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
	EventVal*	UsesUID(int UID);
...
struct	SelExpr:Expr
{
	...
	int			Evaluate(int);
 	...
 	Expr*	UsesGlobRef(int GRI);
	EventVal*	UsesUID(int UID);
	int			DeRefGlobRefs()		{base.DeRefGlobRefs();stepsize.DeRefGlobRefs();steps.DeRefGlobRefs();div.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
...
struct	WorldExpr:Expr
{
	...
	int			Evaluate(int formpos)	{return uniqueID.Evaluate();}
 	...
 	Expr*	UsesGlobRef(int GRI);
	EventVal*	UsesUID(int UID);
	int			DeRefGlobRefs()		{uniqueID.DeRefGlobRefs();delta.DeRefGlobRefs();rotate.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
...
struct	XyzExpr:Expr
{
	...
	UsesType	UsesCallerType(int)		{return EUCT_int;}
 	...
 	Expr*	UsesGlobRef(int GRI);
	EventVal*	UsesUID(int UID);
	int			DeRefGlobRefs()		{X.DeRefGlobRefs();Y.DeRefGlobRefs();Z.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
...
struct	RotExpr:Expr
{
	...
	UniqueID	EvalW(Coords3D& c);
 	...
 	Expr*	UsesGlobRef(int GRI);
	EventVal*	UsesUID(int UID);
	int			DeRefGlobRefs()		{icpt.DeRefGlobRefs();range.DeRefGlobRefs();hdg.DeRefGlobRefs();pitch.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
...
struct	IcptExpr:Expr
{
	...
	UsesType	UsesCallerType(int)		{	return	EUCT_int;}
 	...
 	Expr*	UsesGlobRef(int GRI);
	EventVal*	UsesUID(int UID);
	int			DeRefGlobRefs()		{objA.DeRefGlobRefs();objB.DeRefGlobRefs();return GOT_UNDEFINED_VAL;}
