...
#ifdef NDEBUG
#define	WindowProc	DisableTheWindowProc
#endif
...
{
	int	resnum;
	RString(int res)	{resnum=res;}
	operator CString()	{return LoadResString(resnum);}
};
...
struct	PolyList
{
	...
	~PolyList();
	void	Clear();
	//Default is do 1 poly. If you put -1 I look for emp
	void	Add(PolyOutLine*,int howmany=1);
	void	operator = (PolyOutLine::MyControl w)	{list=w;}
	void	operator += (PolyOutLine&p)	{Add(&p,1);}
	void	operator += (PolyOutLine*p)	{Add(p,1);}
	void	Scale(int source,int target);
};
class	IconDescUI;
...
public:
	...
	static int RMessageBox(CString TitleID, CString MessageID, UINT Button0ID=NULL,UINT Button1ID=NULL, UINT Button2ID=NULL, FileNum art=FIL_NULL);
...
#define	cloakref(realname,localname)	\
	struct	localname;					\
	typedef	localname	*localname##Ptr;

#define cloakdef(realname,localname)							\
	struct	localname: public realname	{};						\
