...
// Note that this code is only used for the designer. 
// The string name and the actual ID value are both stored internally and the value is used at runtime! 
//
//
...
int GetResourceNumberFromID(const char* text)
{
	...
	// \bob\src\mfc\resource.h
	FILE* fp;
	fp=fopen("\\bob\\src\\mfc\\resource.h","rb");
	if (fp==NULL) 
	{
		MessageBox(AfxGetMainWnd()->m_hWnd,"Cannot find \\bob\\src\\mfc\\resource.h","ERROR",MB_OK);
		return -1;
	}
	// now parse it
	char c=0;
...
{
	// ok first open bob.rc which we'll assume is in
	// \bob\src\mfc\bob.rc
	FILE* fp;
	fp=fopen("\\bob\\src\\mfc\\bob.rc","rb");
	if (fp==NULL) 
	{
		MessageBox(AfxGetMainWnd()->m_hWnd,"Cannot find \\bob\\src\\mfc\\bob.rc","ERROR",MB_OK);
		return FALSE;
	}
	// now parse it
	char c=0;
