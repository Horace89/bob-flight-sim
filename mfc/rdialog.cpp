...
// RowanDialog: // RDialog.cpp : implementation file
//
...
#include "listbx.h"
#include "rscrlbar.h"
#include "..\rlistbox\rlistbxc.h"
#include "..\rbutton\rbuttonc.h"
...
{//STATIC
	...
	if (parent!=NULL)
	{
		if ((tree->edges->l&EDGE::ACTIONS_MASK)==EDGE::ACTIONS_ARTCHILD)
			main->Create(IDDS_EMPTYCHILD,parent);
		else
		{
			main->Create(main->MY_IID,parent);
			if (borderwidth==-1)
			{
				CRect inside,outside;
				main->GetClientRect(&inside);
				main->GetWindowRect(&outside);
				borderwidth=(outside.Height()-inside.Height())/2;
...
//		CView* view=frame->GetActiveView();
		if (tree->edges && (tree->edges->l&EDGE::ACTIONS_MASK)==EDGE::ACTIONS_ARTCHILD)
			main->Create(IDDS_EMPTYCHILD,m_pView);
		else 
		if (tree->edges && (tree->edges->l&EDGE::OCCLUDE_MASK)==EDGE::OCCLUDE_EXACT)
			main->Create(IDDS_EMPTYNOBORDER,m_pView);
		else
		{
			main->Create(main->MY_IID,m_pView);	//IDDS_EMPTYDIAL
			if (borderwidth==-1)
			{
				CRect inside,outside;
				main->GetClientRect(&inside);
...
//DEADCODE JIM 05/04/00 	// scale the dialog if necessary due to font size change
//DEADCODE JIM 05/04/00 	if (dialsize.Width()>=640 && dialsize.Height()>=480)		//RJS 08Sep98
//DEADCODE JIM 05/04/00 		main->m_scalingfactor=1;								//RJS 08Sep98
//DEADCODE JIM 05/04/00 	else														//RJS 08Sep98
...
//	dial->InitArtwork(tree->art);
	...
	// scale the dialog if necessary due to font size change
//DeadCode MS 28Oct100 	if (dialsize.Width()>=640 && dialsize.Height()>=480)	
//DeadCode MS 28Oct100 		dial->m_scalingfactor=1;							
//DeadCode MS 28Oct100 	else													
//DeadCode MS 28Oct100 		dial->m_scalingfactor=(float)fontdpi/96;
	dial->ScaleDialog(1);
	...
	//New code that caches coordinates (if I get it right...)
	if (parent==NULL && (tree->dial->MY_IID!=IDDS_testch1))
	{
		for (cachedlocation=0;cachedlocation<Save_Data.nextdialposmem;cachedlocation++)
			breakif (Save_Data.dialposmem[cachedlocation].ID==tree->dial->MY_IID);
		if (cachedlocation<Save_Data.nextdialposmem && Save_Data.dialposmem[cachedlocation].l!=-0x8000)
		{
			dialsize.left=Save_Data.dialposmem[cachedlocation].l;
			dialsize.top=Save_Data.dialposmem[cachedlocation].t;
			dialsize.right=Save_Data.dialposmem[cachedlocation].r;
			dialsize.bottom=Save_Data.dialposmem[cachedlocation].b;
		}
		else
		{
			Save_Data.dialposmem[cachedlocation].ID=tree->dial->MY_IID;
			Save_Data.dialposmem[cachedlocation].l=-0x8000;
			Save_Data.nextdialposmem++;

		}
	}
	if (dialsize.left<dialsize.right && dialsize.top<dialsize.bottom)	//Save_Data.dialposmem[cachedlocation].l!=-0x8000)
	{
		dial->RECT_NORMAL=CRect(0,0,dialsize.Width(),dialsize.Height());
		if (tree->edges && (tree->edges->l&EDGE::OCCLUDE_MASK)==EDGE::OCCLUDE_EXACT)
...
//		TRACE1("BorderWidth: %d\n",borderwidth);
		...
		if (dialxposflag || dialyposflag)
		{
			...
			default_neverreferenced;
 			case POSN_MIN:
				delta=apppos.left+4;								  //RDH 04/01/00
				break;
			case POSN_CENTRE:
				delta=apppos.left+(apppos.Width()-dialsize.Width())/2;
				break;
			case POSN_MAX:
				delta=apppos.right-dialsize.Width()-4;				  //RDH 04/01/00
				break;
			...
			{
 			case POSN_MIN:
				delta=apppos.top+4;									  //RDH 04/01/00
				break;
			case POSN_CENTRE:
				delta=apppos.top+(apppos.Height()-dialsize.Height())/2;
				break;
			case POSN_MAX:
				delta=apppos.bottom-dialsize.Height()-4;			  //RDH 04/01/00
				break;
			}
			dialsize.top+=delta;
	...
	if (cachedlocation>=0 && Save_Data.dialposmem[cachedlocation].l==-0x8000)
	{
		Save_Data.dialposmem[cachedlocation].l=dial->viewsize.left;
		Save_Data.dialposmem[cachedlocation].r=dial->viewsize.right;
		Save_Data.dialposmem[cachedlocation].t=dial->viewsize.top;
		Save_Data.dialposmem[cachedlocation].b=dial->viewsize.bottom;
	}

	GetDesktopWindow()->GetWindowRect(rect);
	int w=0;
	if (	main->viewsize.left>rect.right-30 )
	{
		w=main->viewsize.Width();
		main->viewsize.left=rect.right-w/2;
		main->viewsize.right=main->viewsize.left+w;
	}
	else
	if (	main->viewsize.right<rect.left+30)
	{
		w=main->viewsize.Width();
		main->viewsize.left=rect.left-w/2;
		main->viewsize.right=main->viewsize.left+w;
	}
	if (	main->viewsize.top<rect.top+20)
	{
		w=main->viewsize.Height();
		main->viewsize.top=rect.top;
		main->viewsize.bottom=main->viewsize.top+w;
	}
	else
	if (	main->viewsize.top>rect.bottom-30)
	{
		w=main->viewsize.Height();
		main->viewsize.top=rect.bottom-w/2;
		main->viewsize.bottom=main->viewsize.top+w;
	}
...
{
//DeadCode JIM 20Oct00 	int	usedy=homesize.Height();
	{
		CRTabs* tabControl;
...
// So find the top.
	if (dragstate!=DRAG_SHUTDOWN)
	{
		...
		else
		{
			...
			{
				int id=top->fchild->MY_IID;
				if (id!=IDDS_testch1)
				{
					int	finddial;
					for (finddial=0;finddial<Save_Data.nextdialposmem;finddial++)
						breakif (Save_Data.dialposmem[finddial].ID==id);
					if (finddial<Save_Data.nextdialposmem)
					{
						CRect r;
						top->GetWindowRect(&r);
						Save_Data.dialposmem[finddial].l=r.left;
						Save_Data.dialposmem[finddial].r=r.right;
						Save_Data.dialposmem[finddial].t=r.top;
						Save_Data.dialposmem[finddial].b=r.bottom;
					}
				}
...
//	ASSERT_KINDOF(CRTabs,tabControl);
	...
	if (list->idtype == IdEntry::T_IDT) text.LoadString(list->textnum);
		...
		if (list->idtype == IdEntry::T_CHAR) text = list->textstr;
...
//DEADCODE RDH 26/11/99 					EndPaint(&paintstruct);
					return;
				}
			}
		}
...
/*	if (parent->dragstate==DRAG_DIALOG)
	...
	if (polylist.first)
	{
		int px=p.x*polylist.scale;
		int py=p.y*polylist.scale;
		for (PolyOutLine*	pl=polylist.first;pl;pl=pl->next)
		{
			if (PtInRegion(pl->regionid,px,py))
			{	//AT PRESENT I AM ASSUMING LIST BOX OR BUTTON
				//In principle, the list box may be a combo box or spin button
				//				the button may be a static or editbutton
				//
				if (polylist.list)
				{
					if (pl->event>0)
						((CRListBox*)(polylist.list))->SetHilightRow(pl->event);
					else if (pl->event<0)
						((CRListBox*)polylist.list)->SetHilightCol(-pl->event);
					else
					{
						((CRListBox*)polylist.list)->SetHilightRow(0);
						((CRListBox*)polylist.list)->SetHilightCol(0);
					}
				}
				else if (pl->button)
				{
	...
	if (polylist.first)
	{
		...
		for (PolyOutLine*	pl=polylist.first;pl;pl=pl->next)
		{
			...
			{	//AT PRESENT I AM ASSUMING LIST BOX OR BUTTON
				...
				//				the button may be a static or editbutton
				//
				int ctrlid,command,numparams=0,param1=0,param2=0;
				if (polylist.list)
				{
					ctrlid=polylist.list->GetDlgCtrlID();
					command=CRListBoxCtrl::eventidSelect;
					numparams=2;
					if (pl->event<=0)
						param1=-pl->event;
					else
				...
				EXCEPINFO	e={0};
				UINT	i=0;
				AFX_EVENT event(AFX_EVENT::event,command,&dispparams,&e,&i);
				OnCmdMsg(ctrlid,CN_EVENT,&event,NULL);
				return;
			}
		}
	}
...
//				ShowScrollBar(SB_VERT,TRUE);
				if (!m_pVertScrollBar)
				{
					...
					m_pVertScrollBar->SetParentPointer((long)this->m_hWnd);
...
//				ShowScrollBar(SB_HORZ,TRUE);
				if (!m_pHorzScrollBar)
				{
					...
					m_pHorzScrollBar->SetParentPointer((long)this->m_hWnd);
...
long RDialog::OnGetXYOffset()
{
	...
	{
//DEADCODE RDH 03/11/99 		CWnd* title;
//DEADCODE RDH 03/11/99 		title=top->fchild->GetDlgItem(IDJ_TITLE);
//DEADCODE RDH 03/11/99 		if (title)
//DEADCODE RDH 03/11/99 		{
//DEADCODE RDH 03/11/99 //			TRACE0("Detected Title\n");
//DEADCODE RDH 03/11/99 			CRect titlerect;
//DEADCODE RDH 03/11/99 			title->GetClientRect(titlerect);
//DEADCODE RDH 03/11/99 			offsety+=titlerect.Height();
//DEADCODE RDH 03/11/99 		}
		//delete title;
	}
...
//			int id=pControl->GetDlgCtrlID();
	...
	if (factor!=m_scalingfactor)
	{	INT3;
		float divfact=factor/m_scalingfactor;
		m_scalingfactor=factor;
		CRect rect;
		CRect parentrect;
		CWnd* pControl;
...
//			int id=pControl->GetDlgCtrlID();
	...
	//		else
		...
		GetParent()->ClientToScreen(parentrect);
		rect-=parentrect.TopLeft();
		MoveWindow(rect.left,rect.top,rect.Width()*divfact,rect.Width()*divfact);

		RECT_NORMAL.right=	RECT_NORMAL.left+	RECT_NORMAL.Width()*divfact;
		RECT_NORMAL.bottom=	RECT_NORMAL.top+	RECT_NORMAL.Height()*divfact;
		homesize.right=		homesize.left+	homesize.Width()*divfact;
		homesize.bottom=	homesize.top+	homesize.Height()*divfact;
	}
	else
	{
...
{
	return RDialog::RMessageBox(LoadResString(TitleID),LoadResString(MessageID),Button0ID,Button1ID,Button2ID,art);
}
int RDialog::RMessageBox(CString TitleID, CString MessageID, UINT Button0ID, UINT Button1ID, UINT Button2ID, FileNum art)
{
	...
	if (art)
		...
		m_pMessageBox->m_artnum=FIL_D_BLANK;
...
//Disable tool bars
	...
	{
		if (main->toolbars[t])
			main->toolbars[t]->EnableWindow(FALSE);
	}}
//DeadCode DAW 11Jul00 	main->m_titlebar.EnableWindow(FALSE);
//DeadCode DAW 11Jul00 	main->m_toolbar1.EnableWindow(FALSE);
//DeadCode DAW 11Jul00 	main->m_toolbar2.EnableWindow(FALSE);
//DeadCode DAW 11Jul00 	main->m_toolbar3.EnableWindow(FALSE);
//DeadCode DAW 11Jul00 	main->m_toolbar4.EnableWindow(FALSE);
//DEADCODE RDH 08/11/99 	main->m_toolbar5.EnableWindow(FALSE);
...
//Enable tool bars
	...
	{
		if (main->toolbars[t])
			main->toolbars[t]->EnableWindow(TRUE);
	}}
//DeadCode DAW 11Jul00 	main->m_titlebar.EnableWindow(TRUE);
//DeadCode DAW 11Jul00 	main->m_toolbar1.EnableWindow(TRUE);
//DeadCode DAW 11Jul00 	main->m_toolbar2.EnableWindow(TRUE);
//DeadCode DAW 11Jul00 	main->m_toolbar3.EnableWindow(TRUE);
//DeadCode DAW 11Jul00 	main->m_toolbar4.EnableWindow(TRUE);
//DEADCODE RDH 08/11/99 	main->m_toolbar5.EnableWindow(TRUE);
	if (main->m_wndSystemBox)
...
//DEADCODE DAW 23/11/99 	if (MY_IID == IDD_FOOTAGE)
//DEADCODE DAW 23/11/99 	  return (1);

	if (parent)
		if (
				parent->MY_IID==IDDS_EMPTYDIAL
...
#ifndef NDEBUG
	// TODO: Add your specialized code here and/or call the base class
//DeadCode JIM 11Oct00 static int r=0;
//DeadCode JIM 11Oct00 	if ( message!=0x113)  //message!=0x363 &&
//DeadCode JIM 11Oct00 	{
//DeadCode JIM 11Oct00 		Mono_Text.PrintAt(30,r,(UByte*)"   ");
//DeadCode JIM 11Oct00 		r++;
//DeadCode JIM 11Oct00 		if (r==25) r=0;
//DeadCode JIM 11Oct00 		Mono_Text.PrintAt(30,r,(UByte*)"<--");
//DeadCode JIM 11Oct00 		Mono_Text.PrintHexAt(0,r,message,8);
//DeadCode JIM 11Oct00 		Mono_Text.PrintHexAt(10,r,wParam,8);
//DeadCode JIM 11Oct00 		Mono_Text.PrintHexAt(20,r,lParam,8);
//DeadCode JIM 11Oct00 	}
...
void	PolyList::Clear()
{
	PolyOutLine*	pl=first;
	current=first=NULL;
	list=NULL;
	while (pl)
	{
		if (pl->regionid)
			DeleteObject(pl->regionid);
		pl->regionid=0;
		PolyOutLine* p2=pl->next;
		pl->next=NULL;
		pl=p2;
	}
...
void	PolyList::Add(PolyOutLine* newentries,int howmany)
{
	while(howmany--)
	{

		newentries->next=first;
		first=newentries;
		int numpoints=0;
		while (++numpoints<10)
			if (newentries->points[numpoints].x==0 && newentries->points[numpoints].y==0)
				break;
		newentries->regionid=CreatePolygonRgn(newentries->points,numpoints,ALTERNATE);
		newentries++;
		if (howmany<0)
			breakif(newentries->button==NULL && newentries->event==0);
	}
}

void RDialog::OnRButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	
//TempCode JIM 19Oct00 	CDialog::OnRButtonDown(nFlags, point);
}
