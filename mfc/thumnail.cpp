...
void CThumbnail::OnPaint() 
{
	...
	// +1 prevents gaps
//DeadCode JIM 20Oct00 	int halfheight=rect.Height()/2+1;
//DeadCode JIM 20Oct00 	int offset=rect.Height()/8+1;
//DeadCode JIM 20Oct00 	int offset2=rect.Height()/16+1;
	if (rect.Width()<400)
	{
		DrawScaledBitmap(pDC,FIL_MAP512_11,rect.left,rect.top,rect.Width(),rect.Height());

	}
	else
	{
		DrawScaledBitmap(pDC,FIL_MAP256_15,rect.left,				rect.top,					rect.Width()/2+1,rect.Height()/2+1);
		DrawScaledBitmap(pDC,FIL_MAP256_55,rect.left+rect.Width()/2,rect.top,					rect.Width()/2+1,rect.Height()/2+1);
		DrawScaledBitmap(pDC,FIL_MAP256_11,rect.left,				rect.top+rect.Height()/2,	rect.Width()/2+1,rect.Height()/2+1);
		DrawScaledBitmap(pDC,FIL_MAP256_51,rect.left+rect.Width()/2,rect.top+rect.Height()/2,	rect.Width()/2+1,rect.Height()/2+1);
	}
